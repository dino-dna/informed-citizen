...
const DESIRED_DOMAIN_CATEGORIES: DomainCategories[] = [
  'credible',
  'trusted'
]
  ...
  return (
    <div className={`${className} article_analysis_card--container`} {...rest}>
      <div style={{display: 'flex', alignItems: 'center'}}>
        ...
        {isBiased
          ? <span className='article_analysis_card--first-glance --warning'>article is <span style={{color:'red', padding: 4}}>biased</span></span>
          : <span className='article_analysis_card--first-glance'>article is {contentDecision}</span>
        }
      </div>
      ...
      <table>
        <tbody>
          ...
          <tr>
            <td>content</td>
            <td style={{ color: contentScore > 0.8 ? 'green' : 'red'}}>{(contentScore * 100).toFixed(0)}%</td>
            <td>{contentDecision}</td>
          </tr>
          <tr>
            <td>title</td>
            <td style={{ color: titleScore > 0.8 ? 'green' : 'red'}}>{(titleScore * 100).toFixed(0)}%</td>
            <td>{titleDecision}</td>
          </tr>
        </tbody>
      </table>
      <div style={{display: 'flex', alignItems: 'center'}}>
        <ScoringThumb score={isGoodDomain ? 1 : 0} style={{height: 18, width: 18}} />
        {isGoodDomain
          ? <span style={{padding: 4}}>the article domain is {category}</span>
          : <span style={{color: 'red', padding: 4}} >
              the article domain is not one of "{DESIRED_DOMAIN_CATEGORIES.join(', ')}".
              it is {category}.
            </span>
        }
      </div>
